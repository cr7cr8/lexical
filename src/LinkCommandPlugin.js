                    ...
                    if (node.__type !== "text") {
                        ...
                        editor.update(function () {
                            ...
                            if (selection.anchor.key !== node.__key && selection.focus.key !== node.__key) {
                                ...
                                //  if (range.startContainer.nodeName !== "#text" || range.endContainer.nodeName !== "#text") { return }
...
export function LinkButton() {
    ...
    const [hasMentionNode, setHasMentionNode] = useState(false)

    //$selectAll(selection): void

    function checkMentionNode() {
        setHasMentionNode(false)
        editor.getEditorState().read(() => {
            const selection = $getSelection()
            if (!selection) return
